...
hbox margin@(marginX, marginY) hseparator children = UiElement {
...
vbox margin@(marginX, marginY) vseparator children = UiElement {
  name = "hbox " ++ (show children),
  size = bounds,
...
layoutHorizontal (sizeX, sizeY) (marginX, marginY) hseparator children w = do
  ...
  return $ Translate marginX marginY $ Pictures $ map (\(off, p) -> Translate off 0 p) $ zip offsets pics
...
layoutVertical (sizeX, sizeY) (marginX, marginY) vseparator children w = do
  ...
  return $ Translate marginX marginY $ Pictures $ map (\(off, p) -> Translate 0 off p) $ zip offsets pics
